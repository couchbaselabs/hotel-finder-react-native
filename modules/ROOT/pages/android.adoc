...
image::android_flow.jpg[,50%]

== Architecture

The user Interface is written in JavaScript while the business logic and data model is written in native Java . The data model uses Couchbase Lite as the embedded data persistance layer. https://facebook.github.io/react-native/docs/native-modules-android  [React Native module] acts as the bridging layer between the Javascript layer and the native java layer. 
...
----
. Open the *MainApplication.java* and  update the `getPackages` method to return the *HotelFinderPackage* package
+
[source,java]
----
...
- Invoke the exported method from JS (you will have to import the ReactNative module the very first time)
...
As an exercise, you can follow the same procedure to implement 
