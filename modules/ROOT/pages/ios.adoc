...
The user Interface is written in JavaScript while the business logic and data model is written in native Swift.
The data model uses Couchbase Lite as the embedded data persistence layer.
https://facebook.github.io/react-native/docs/native-modules-ios[React Native module] acts as the bridging layer between the Javascript layer and the native Swift layer.
