...
image::ios_flow.jpg[,50%]

== Architecture

The user Interface is written in JavaScript while the business logic and data model is written in native Swift . The data model uses Couchbase Lite as the embedded data persistance layer. https://facebook.github.io/react-native/docs/native-modules-ios[React Native module] acts as the bridging layer between the Javascript layer and the native swift layer. 
